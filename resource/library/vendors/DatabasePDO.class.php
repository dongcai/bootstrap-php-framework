...
/*
 *  Per PHP.NET, mysql_connect is deprecated as of PHP 5.5.0, 
 *  and will be removed in the future. 
 *  Instead, the MySQLi or PDO_MySQL extension should be used. 
 * 
 *  So here is MySQL PDO replacement written by Max Fierke.
 *  
 */
...
class DatabasePDO {
    ...
    private function __construct() {
        ...
        $this->name = DB_NAME;
        $this->onError = false;
        $this->emailOnError = false;
        $this->queries = array();
        // MySQL specific stuff 
        if ($this->type == 'mysql') {
            $this->writeHost = DB_HOST;
    ...
    public function rowCountResult() {
        if (!$this->isMySQL()) { // For SQLite only.
            ...
            return $this->result->rowCount(); // The last query was not a SELECT query. Return the row count normally.
    }
